...
- [ ] 연합회 등급에 `adm/villages`·`adm/contest`를 열어 두었는데 두 화면에 편집 버튼(마을 추가·담당자 배정 등)이 그대로 있다 — 연합회는 열람만 하게 할지 사용자 결정. 열람 전용이면 `admVillages()`에서 `currentUser().role !== 'admin'`일 때 버튼 숨기기
...
| ④ 위키배포 | v4 **819b77c** → CI 36092761197 success(build 잡 직후 아티팩트 API로 수령) → site master **c183bfb**(sitemap `<loc>` 120) = **3단계 완료**. 라이브(Caddy)는 13:05 서버 실행분 **119**에서 멈춰 신규 문서 404 → **4단계 서버 반영 대기**: `bash /opt/sakyowon/src/deploy-www.sh` |
