...
- [x] **해소(18:55)**: `hatsoja/manual.html`에 `#roles` 「회원등급 4단계」 절 추가 — PR #5 → main 머지. 목차·데모 4종·등급별 미리보기·문제해결 안내문 3종 갱신. 작업은 hub-clone이 다른 세션 브랜치라 **`git worktree`로 origin/main 별도 체크아웃**해서 했음(스킬에 기록). **라이브 반영 완료** — 2026-09-26 12:33 KST 사용자 서버 실행(`Last-Modified` 03:33:05 GMT), 라이브 manual.html에 `#roles` 절·목차 링크·데모 4종·문제해결 안내문 확인, 브라우저에서 표 4행·h3 3개 렌더 확인
