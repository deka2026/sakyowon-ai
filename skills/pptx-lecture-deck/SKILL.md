...
- 슬라이드는 16:9 = **12192000 × 6858000 EMU**(13.333×7.5"), 좌우 여백 0.55", 본문 폭 12.23". `new_deck()`이 이 값과 `sldSz type="screen16x9"`를 함께 설정한다(함정 8)
...
8. **`Inches(13.333)`만 넣으면 16:9가 아니다** (2026-09-14). python-pptx 기본 템플릿의 `<p:sldSz type="screen4x3">`가 남고 cx도 12191695로 305 EMU 짧아, PowerPoint 밖 뷰어·행사장 장비가 4:3/사용자 지정으로 인식할 수 있다. 크기는 정수 EMU(12192000×6858000)로, `sldSz`의 type은 `screen16x9`로 직접 고친다. 기존 파일 정정 시 폭이 옛 슬라이드 폭과 같은 전폭 도형(left=0)도 함께 넓힌다. PowerPoint COM의 `PageSetup.SlideSize`는 기본 와이드스크린도 7(custom)을 돌려주므로 판별 지표로 쓰지 말고 `SlideWidth/Height`(960×540pt)와 XML을 본다.
