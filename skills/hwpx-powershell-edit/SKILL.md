...
## 경로 I — 낱권 여러 권을 한 권으로 합치기 (2026-09-25 실증)

"세 자료 합쳐서 한 파일로" 요청. **본문을 복사해 붙이지 말 것** — 낱권을 고칠 때마다 합본이
갈라진다. 낱권 build 스크립트의 **본문 구간만 떼어 내 하나의 doc에 exec**하면, 낱권을 고친 뒤
합본 스크립트를 다시 돌리기만 하면 된다. 학습자료 3권(11+10+10쪽, 표 90개) → 합본 32쪽·표 93개로 실증.

```python
MARK = '# ' + '=' * 69            # 각 build 스크립트의 구분선

def body_of(fname):
    seg = open(fname, encoding='utf-8').read().split(MARK)
    assert len(seg) >= 3           # 장마다 구분선을 쓴 스크립트는 조각이 여럿
    return MARK.join(seg[1:-1]).replace('d.title(', 'PT(')

ns = dict(d=d, T=T, PT=PT, esc=esc, W=W)
for f in [...]:
    exec(compile(body_of(f), f, 'exec'), ns)
```

- **`title()`은 합본 전체에서 한 번만** 호출한다. secPr·colPr을 그 문단이 운반하므로 두 번 부르면
  쪽 설정이 중복된다. 각 부 제목은 쪽 나눔 + 제목 서식 문단으로 따로 낸다:

```python
def PT(t):
    st = STYLES['title']
    d._p(st['parapr'], st['style'],
         '<hp:run charPrIDRef="%s"><hp:t>제%d부.  %s</hp:t></hp:run>' % (st['charpr'], n, esc(t)),
         page_break=True)          # patch_table로 감싼 doc도 _p는 그대로 쓸 수 있다
```

- 합본 앞에는 **부별 구성 · 상황별 읽는 순서 · 부 사이의 관계 대조표**를 붙인다. 낱권을 그냥 이어
  붙이기만 하면 "왜 같은 낱말이 세 번 나오나"라는 물음에 답이 없다.
- `audit`의 `tables`가 낱권 합계 + 앞표 수와 맞는지로 누락을 확인한다(예: 29+30+31+3 = 93).
- 낱권 스크립트의 구분선 규칙이 서로 다를 수 있다(장마다 `=` 줄을 쓴 것과 `-` 줄을 쓴 것).
  `seg[1]`만 쓰면 첫 장만 들어가므로 반드시 `seg[1:-1]`을 이어 붙인다.

### 목표 쪽수가 모자랄 때 (경로 E의 반대 방향)

심화 자료가 기초편보다 얇게 나오면 분량을 늘린다. 잡문을 붙이지 말고 **실무에서 실제로 찾아보는
표**를 넣는 것이 낫다 — 진단 체크리스트(증상 → 원인), 계보·연표, A와 B 비교표, 점검 목록.
2026-09-25에 8쪽짜리에 이런 표 4개를 더해 10쪽을 맞췄고, 내용 가치도 함께 올랐다.

### 경로 I·H 함정 (2026-09-25 실전)

1. **`esc()`는 `&`만 이스케이프하므로 소스에 `&amp;`를 직접 쓰면 이중 이스케이프된다.**
   `'O&amp;M'` → 저장 시 `O&amp;amp;M` → 문서에 **`O&amp;M`이라는 글자가 그대로 보인다**.
   소스에는 언제나 `O&M`, `R&D`처럼 맨 `&`로 적는다. 생성 후 점검:
   `zipfile로 section0.xml을 읽어 s.count('&amp;amp;') == 0` 확인.
2. **`hwp_verify.ps1`의 TEXT 덤프는 cp949다.** UTF-8로 읽으면 한국어가 전부 깨져 검증 키워드가
   하나도 안 맞고 "본문 누락"으로 오판한다. `raw.decode('cp949', errors='replace')`로 읽을 것.
   ASCII 키워드만 맞고 한국어만 전부 틀리면 십중팔구 이 문제다.
3. **유령 Hwp 프로세스로 첫 호출이 RPC 오류(0x800706BA)로 죽을 수 있다.** 앞선 워커가 남긴
   프로세스가 응답하지 않는 상태다. `run_auto.ps1`은 "시작 전부터 있던 것"으로 보고 건드리지
   않으므로, `Get-Process Hwp`로 실체가 없어진 것을 확인하고 **그냥 한 번 더 실행**하면 된다.
   사용자 문서가 열려 있는 게 아니라면 죽일 필요도 없다.
4. **PowerShell/Bash 콘솔로 한국어를 print하면 cp949 인코딩 오류나 깨진 글자가 나온다.**
   파일 내용 검증 결과를 출력할 때는 `PYTHONIOENCODING=utf-8`을 붙인다. 출력이 깨졌다고
   파일이 깨진 것으로 보고하지 말 것.
