...
이월 과제였다. 설립제안서(`전남광주해바람물자산재단_설립제안서_20260912.hwpx`) 본문을 전수 확인한 결과 **7곳 모두 "조직 300만 원 이상 · 개인 100만 원 이상"으로 이미 통일**돼 있었고, 폴더 전체 hwpx에 "300백만원" 표기는 없었다. **사용자 확정: 300백만원은 300만원의 오기.** 정정할 문서 없음. 구조적으로도 조직:개인 = 3:1이 자연스럽고, 3억이면 300배 차이라 짝이 성립하지 않는다.

**작업 방식 교훈**: 원천 표기를 찾겠다고 폴더 전체 바이트 스캔까지 벌였는데, 사용자가 답을 알고 있어 중단시켰다. **기록에 "사용자 미확인"으로 남은 값은 탐색보다 질문이 먼저다.**

### 7. 새 스킬 — `server-runtime-upgrade`

서버 런타임을 기존 서비스를 깨지 않고 올리는 절차. 이번 Node 22 작업에서 확립한 판단 경로를 그대로 패키지화했다. 핵심은 **systemd 유닛의 ExecStart 경로를 읽어 영향 여부를 판정**하는 것(systemd는 `~/.bashrc`를 읽지 않으므로 버전 매니저와 무관). 함정 4종 포함 — 어느 기계인지 확인시키기, `EBADENGINE`은 차단이 아니라 경고, 앱 내장 터미널 전역 설치 금지, 유닛 이름으로 언어 짐작 금지.

### 8. 레슨 2편째

`연대지능/서버를-깨지-않고-버전-올리기` — 단체 서버 한 대에 여러 서비스가 얹힌 상황에서 "올리지 말고 덧대는" 판단을 활동가 언어로 풀었다. 검증은 새 것이 아니라 **옛 것이 그대로인지**를 본다는 게 골자.

배포: v4 `c291503` → CI 35324420881(아티팩트 10538371474, sitemap **108**) → site master `fcc33ac` = **3단계 완료**, 4단계 서버 반영 대기(편지 `6994a7f`). 오늘 17:12 반영분 이후 push라 `deploy-www.sh` 1회 추가 실행 필요 — 그 외 대기분은 없다.
