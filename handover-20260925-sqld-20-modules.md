...
- [x] (2026-09-26) 2과목 회차마다 계산형 2문항 추가(28개, 총 148문항). 노랭이 실물 대조는 아직 — 사용자가 문제집을 주면 유형 비교 가능
- [x] (2026-09-26) `yt` 검색어에 채널명(이기적 / 홍쌤의 데이터랩) 포함
- [x] (2026-09-26 사용자 결정) 자격증 과정 데이터는 **index.html 단일 파일 유지**. 분리하지 않는다
- [x] (2026-09-26) 홈 AI 대화 `KB` 29항목 현행화(옛 페이지 이름·6단계 제거, SQLD·유료 계정·다음 버튼·하네스 추가) / 9↔10단계는 교안 순서 유지한 채 프롬프트로 연결(10단계에서 CLAUDE.md 복사 + 같은 저장소 대체 push, 9단계 첫 빌드 실패 정상 안내)
- [x] (2026-09-26 사용자 결정) 캡처 올리기 **제거**, "내 클로드 대화창에 올리세요" 안내로 통일. 업로드 함수 2개 삭제
- [x] (2026-09-26) 학습개요 정리: Supabase 카드 절차→개념+실습 없음 표시, 프론트엔드 예시 프롬프트 별개 표시, 요금 문단 준비물 참조, API 카드 주의. **7단계 API 키 설정을 선택 단계로**(구독 로그인이면 불필요, 키 영구 등록 시 API 종량 요금 주의)
...
## 이어서 작업 (2026-09-26 오전)

- academy-site main **(커밋은 git log 참조, 메시지 "이어서 작업: SQLD 계산형 문항 28개 추가 …")**: 위 [x] 4건 처리. 검증: 계산형 문항 topic↔카드 연결 검사 ALL OK, 스크립트 블록별 `node --check`, 로컬 프리뷰에서 12회차 8문항·3회차 6문항·버튼 라벨 동적·KB 항목 응답 확인
- 함정 기록: **`python - <<'EOF'` 히어독에 한국어 리터럴을 넣으면 stdin이 cp949로 읽혀 매칭이 실패**할 수 있다(같은 세션에서 됐다 안 됐다 함). 한국어 치환 스크립트는 **파일로 저장해 `python 파일.py`로 실행**한다(`%TEMP%cad_patch\`). 스킬 academy-site-deploy에 반영
- 서버 반영 대기 → 사용자에게 `deploy-www.sh` 한 줄 안내

- 2026-09-26 오후: 사용자 결정 2건 반영(캡처 제거·단일 파일 유지) push 완료. 서버 반영 대기

- 2026-09-26 오후: 캡처 제거(527f6d1) 12:26 라이브 확인. 학습개요 정리 + 7단계 선택화 push(커밋은 git log "학습개요·실습 카드 역할 분담 정리"). **아카데미 미완료 0건**(서버 반영만 대기). 발견: 유료 구독 전제와 7단계 API 키 영구 등록이 충돌(구독 대신 API 과금) — 교안 3부 15단계 지도에도 7단계를 "선택"으로 표기할 것을 다음 교안 갱신 때 반영

## 사이드 메뉴 역할별 구분 (2026-09-26 오후, academy-site main "사이드 메뉴를 역할별…" 커밋)

- 정적 `<nav>` → `SIDEBAR_MENUS` + `renderSidebar()`(applyRole에서 호출). `currentRole()`이 관리자의 미리보기(sessionStorage `academy_preview_role` = user|guest)를 반영하고 `isRealAdmin()`이 실제 역할. `sidebarPreview(role|null)`로 전환, 로그아웃 시 해제. `go()`의 adminPages에 admin-dashboard 추가.
- 검증: 손님/교육생/관리자/미리보기 2종/복귀, 손님 미리보기에서 관리자 페이지·게임 접근 차단 메시지, 페이지 이동 시 active 표시, 콘솔 오류 없음.
- 사용자에게 이용자 불편 요소 12건 보고(채팅). 사용자가 고르면 다음 작업으로.
